...
* Red Hat Enterprise Linux ou CentOS versão 7.x e 8.x, com base na arquitetura x86_64. CentOS Stream não é suportado.
...
NOTE: O NetApp não oferece suporte à instalação do Gerenciador Unificado usando 3rd ferramentas de terceiros, como o Gerenciador de Configuração do Centro de sistema (SCCM) da Microsoft.
...
* MySQL Community Edition versão 8.0.27 (do repositório MySQL).
* OpenJDK versão 11.0.12 (do repositório Red Hat extra Enterprise Linux Server)
* Python 3,6.x
...
+
link:task_manually_configure_epel_repository.html["Configurando manualmente o repositório EPEL"]
...
+
link:task_manually_configure_mysql_repository.html["Configurando manualmente o repositório MySQL"]
